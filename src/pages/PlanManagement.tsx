  ...
  const handleSubscribe = async (planType: BillingPlanRow["plan_type"]) => {
    ...
    try {
      ...
      try {
        ...
        if (lemonRes.error) throw new Error(lemonRes.error.message || "LemonSqueezy edge function error");
        ...
        if (!lemonPayload?.checkout_url) throw new Error("Failed to create LemonSqueezy subscription");
