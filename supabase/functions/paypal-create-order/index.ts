...
serve(async (req) => {
  ...
  try {
    // require logged-in user (consistent with your create-subscription)
    const auth = req.headers.get("Authorization")?.replace("Bearer ", "");
    const { data: { user } } = await supabase.auth.getUser(auth || "");
    if (!user) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: cors });

    const { amount, currency = "USD", reference = "card-order" } = await req.json();
    if (!amount) return new Response(JSON.stringify({ error: "amount required" }), { status: 400, headers: cors });

    const access = await paypalToken();

    // create an Order intended for card payment (no PayPal login)
        ...
        purchase_units: [
          {
            ...
            custom_id: user.id, // tie back to your user
          },
        ],
        payment_source: { card: {} }, // card (Hosted Fields) path
      }),
    });
    ...
    if (!r.ok) {
      ...
      return new Response(JSON.stringify({ error: "PayPal create failed", details: json }), { status: r.status, headers: cors });
    }

    // return the order id to the client (Hosted Fields will submit/capture it)
    return new Response(JSON.stringify({ id: json.id }), { status: 200, headers: cors });
  } catch (e: any) {
    console.error(e);
